...
declare namespace wx {
    ...
    data: { [key:string]: any } | string | ArrayBuffer;
		/** 开发者服务器返回的 HTTP 状态码 */
		statusCode: number;
		/** NOTE: 开发者服务器返回的 HTTP Response Header */
    // header: object;
  ...
  // 创建并返回绘图上下文context对象，新版接口
  function createCanvasContext(canvasId: string, canvasIdInst?: any): CanvasContext;
  ...
  interface cloud {
		 ...
		 * 接受一个可选对象参数 env：环境 ID，获取数据库的引用
		 */
		database: (options?: { env: string }) => databaseInst;
		/**
		 * 接受一个 name 参数，指定需引用的集合名称
		 */
    collection: (name: string) => any;

    callFunction: (opt: callFunctionOptions) => Promise<callFunctionResult>;
  }

  // 数据库实例
  interface databaseInst {
    // NOTE: 具体的数据库命令待补充
    command: any;
    collection: (name: string) => Collection;
    serverDate: (opt?: { offset: number }) => any;
    Geo: {
      Point: (longitude: number, latitude: number) => any;
    }
  }

  // 数据库集合
  interface Collection {
    // NOTE: Document实例接口待补充
    doc: (id: string | number) => any; 
    get: () => Promise<collectionResult>; // 获取集合数据，或获取根据查询条件筛选后的集合数据
    add: (opt: { data: any }) => Promise<{ _id: string | number }>; // 在集合上新增记录，data为新增记录定义
    update: (opt: { data: any }) => Promise<{ stats: { updated: number }}>; // 更新多条记录
    remove: (opt?: any) => Promise<{ stats: { removed: number }}>; // 删除多条记录
    count: () => Promise<{ total: number }>; // 统计集合记录数
    where: (rule: any) =>  any; // 指定筛选条件，腾讯的文档是真的跟屎一样。。。
    orderBy: (fieldName: string, order: 'asc' | 'desc') => this; // 指定查询排序条件
    limit: (max: number) => this; // 指定查询结果集数量上限
    skip: (offset: number) => this; // 指定查询返回结果时从指定序列后的结果开始返回，常用语分页
    field: (definition: any) => this; // 指定返回结果中记录需返回的字段
  }

  // Collection.get 方法获取的结果实例
  ...
  interface callFunctionOptions {
    name: string; // 云函数名
    data?: { [key: string]: any }; // 传递参数
    config?: { [key: string]: any }; // 局部覆写 wx.cloud.init 中定义的全局配置
    success?: (res: callFunctionResult) => void; // 返回云函数调用的返回结果
